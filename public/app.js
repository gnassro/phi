 ...
 *
 * Full-featured UI adapted from Tau for VS Code extension.
 * Key di5fference: ALL communication via VscodeIPC (no WebSocket, no HTTP fetch).
 */
  ...
  switch (event.type) {
    ...
    case 'message_end': {
      ...
      if (currentStreamingEl) {
        ...
        messageRenderer.finalizeStreamingMessage(currentStreamingEl, usage, currentThinking);
        currentStreamingEl = null;
        currentThinking = '';
        currentText = '';
    ...
    case 'auto_compaction_end': {
      ...
      if (indicator) {
        const summary = event.summary || (event.result?.summary) || '';
        indicator.innerHTML = `✓ Context compacted${summary ? ` — ${summary}` : ''}`;
    ...
    if (msg.role === 'user') {
      ...
      if (text || thinkingBlocks.length > 0) {
        messageRenderer.renderAssistantMessage(
          { content: contentBlocks.length > 0 ? contentBlocks : text, usage: msg.usage },
          false, true
        );
